  ...
  try {
    ...
    for (const item of OFFICIAL_POLISH_TYPES) {
      const exist = await db.getFirstAsync<{ isExist: number }>(
        `SELECT EXISTS(SELECT 1 FROM user_polish_types WHERE type_name = ?) as isExist`,
        item,
      )
      ...
      if (exist!.isExist === 0) {
        ...
        await db.runAsync(
          `INSERT INTO user_polish_types (polish_type_id, user_id, type_name) VALUES (?, ?, ?)`,
          [Crypto.randomUUID(), user!.id, item],
        )
      }
    }
    ...
    for (const item of OFFICIAL_COLOR_TYPES) {
      const exist = await db.getFirstAsync<{ isExist: number }>(
        `SELECT EXISTS(SELECT 1 FROM official_color_types WHERE color_type_id = ?) as isExist`,
        item.id,
      )
      ...
      if (exist!.isExist === 0) {
        await db.runAsync(
          `INSERT INTO official_color_types (color_type_id, color_type) VALUES (?, ?)`,
