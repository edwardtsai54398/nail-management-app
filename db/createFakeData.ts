          ...
          if (!(await isDataExists(db, 'user_polish_items', 'polish_id', itemId))) {
            await db.runAsync(
              insertInto('user_polish_items')
                ...
                .colVal([
                  series[j].isTypeOfficial ? 'official_polish_type' : 'user_polish_type',
                  series[j].isTypeOfficial ? series[j].polishType : seriesId,
                ])
                .end(),
            )
            console.log(`items ${series[j].polishType} excecuted`)
            await db.runAsync(
              insertInto('user_polish_stocks')
            ...
            const colorRows = await db.getAllAsync<OfficialColorTypesSchema>(
              `SELECT * FROM official_color_types`,
            )
