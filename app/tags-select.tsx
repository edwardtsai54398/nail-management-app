...
import type {ParamsFromSelection} from "@/components/ui/PolishForm/types";
...
export default function TagsSelect() {
    ...
    const isSelected = useCallback((t: Tag) => tagsSelected.some(tag => tag.tagId === t.tagId), [tagsSelected])
    const handleTagItemPress = useCallback((tag: Tag) => {
        ...
        <>
            <Stack.Screen
                options={{
                    ...
                    headerRight: () => (!isAddMode ? <ThemeButton label="新增標籤" text onPress={() => {setIsAddMode(true)}} /> : <ThemeButton label="取消" text onPress={handleCancelPress} />),
            ...
            {isAddMode ? (
                <Flex style={{padding: SPACING.sm}}>
                    <TextInput
                        ...
                        maxLength={25}
                        numberOfLines={1}
                        onChangeText={(val) => {setNewTagText(val)}}
                        placeholder={'輸入新標籤名稱'}
                        style={[uiStyles.input, {flexGrow: 1, marginRight: SPACING.sm}]}
                    />
                    <ThemeButton text label={'完成'} disabled={(newTagText === '')} onPress={handleFinishPress} />
                </Flex>
            ) : null}
            <SelectList
                data={userTags}
