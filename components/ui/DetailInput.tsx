    ...
    return (
      <Pressable onPress={handlePress}>
        <Flex
          ...
          style={[
            styles.container,
            border === 'top' ? styles.borderTop : border === 'bottom' ? styles.borderBottom : null,
          ]}>
          ...
          {type === 'select' ? (
            <Flex>
              {value ? (
                ...
                <ThemeText color="subtle">{placeholder}</ThemeText>
              )}
              <AntDesign
                name="right"
                size={20}
                style={{ marginLeft: SPACING.sm, color: TEXT_COLORS.second }}
              />
            </Flex>
          ) : (
            <TextInput
